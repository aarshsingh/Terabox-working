...
async def stats_command(m: UpdateNewMessage):
    if m.is_group or m.is_channel:
        return
    check_if = await is_user_on_chat(bot, f"@XBOTSHUB_OFFICIAL", m.peer_id)
    if not check_if:
        return await m.reply(f"Please join @XBOTSHUB_OFFICIAL then send me the link again.")
    check_if = await is_user_on_chat(bot, f"@XBOTSHUB_OFFICIAL", m.peer_id)
    if not check_if:
        return await m.reply(f"Please join @XBOTSHUB_OFFICIAL then send me the link again.")
...
"""
    ...
    await m.reply(
        ...
        buttons=[
            [
                Button.url("Update Channel", "https://t.me/XBOTSHUB_OFFICIAL"),
                Button.url(
...
async def handle_message(m: Message):
    ...
    check_if = await is_user_on_chat(bot, "@XBOTSHUB_OFFICIAL", m.peer_id)
    if not check_if:
        return await m.reply("Please join @XBOTSHUB_OFFICIAL  then send me the link again.")
    check_if = await is_user_on_chat(bot, "@XBOTSHUB_OFFICIAL", m.peer_id)
    if not check_if:
        return await m.reply("Please join @XBOTSHUB_OFFICIAL then send me the link again.")
    is_spam = db.get(m.sender_id)
    if is_spam and m.sender_id not in [6801198172]:
...
print("This bot is deployed by @XBOTSHUB_OFFICIAL kindly join this channel for more updates.")
